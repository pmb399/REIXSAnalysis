...
def grid_data(mcpRSXS_scales,mcpRSXS_scatters,img,x_low=None,x_high=None,y_low=None,y_high=None,new_x=None,new_y=None):
    ...
    xpoints = int(np.abs(np.ceil((xmax-xmin)/np.abs(np.diff(mcpRSXS_scale[0,:])).min())))
    ypoints = int(np.abs(np.ceil((ymax-ymin)/np.abs(np.diff(mcpRSXS_scale[1,:])).min())))
...
def img_to_sca(mcpRSXS_scales,mcpRSXS_scatters,img,x_low=None,x_high=None,y_low=None,y_high=None,axis=0,new_x=None,new_y=None):
    ...
    xmin,xmax,ymin,ymax,new_x,new_y,new_z = grid_data(mcpRSXS_scales,mcpRSXS_scatters,img,x_low,x_high,y_low,y_high,new_x,new_y)
...
def grid_stack(mcpRSXS_scales,mcpRSXS_scatters):
    ...
    for i in range(len(mcpRSXS_scatters)):
        xmin,xmax,ymin,ymax,new_x,new_y,new_z = grid_data(mcpRSXS_scales,mcpRSXS_scatters,i)
...
def stack_to_mca(mcpRSXS_scales,mcpRSXS_scatters,x_low=None,x_high=None,y_low=None,y_high=None,axis=0):
    ...
    sorter_0_min,sorter_0_max,sorter_1_min,sorter_1_max,min_diff_0,min_diff_1 = get_interpolation_boundaries(mcpRSXS_scales)
    ...
    mca = []
    for i in range(len(mcpRSXS_scatters)):
        caxis,newz_sum=img_to_sca(mcpRSXS_scales,mcpRSXS_scatters,i,xmin,xmax,ymin,ymax,axis,new_x,new_y)
        mca.append(newz_sum)
